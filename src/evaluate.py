...
def parse_args():
  ...
  parser.add_argument('--batch_size_ep', type=int, default=1)
  parser.add_argument('--batch_size_opt', type=int, default=1)
  ...
  parser.add_argument('--consistency', type=str, default='N')
  parser.add_argument('--baseline', type=str, default='M')
  parser.add_argument('--exploration', type=str, default='E')
...
def load_checkpoint(num_songs, args):
    ...
    checkpoint = torch.load(f'{args.checkpoints_save}', map_location='cpu')
    ...
    # print(f"Checkpoint loaded: Res/uming from song no. {num_songs}")
...
def step(rank,
  ...
  is_done = terminated
  reward_rule = torch.zeros(batch_size, dtype=torch.float32)
  ...
  for i, j in zip(range(batch_size), current_ids):
    if f'{BAR_KEY}_' in action[1][i] and int(action[1][i].split('_')[-1]) != 1:
      ...
      nd, pm, pstd, vm, vstd, dm, dstd, ts, inst, ch, chroma, groove = get_meta(curr_bar_seq)
      # try:
      ...
      chroma_org = meta_stats[i]['chroma'][current_bar-1]
      groove_org = meta_stats[i]['groove'][current_bar-1]
      ...
      reward_nd = (np.abs(nd - nd_org)**2 / np.mean(meta_stats[i]['note_density'])**2)
      ...
      reward_chroma = cosine_sim(chroma_org, chroma)
      reward_groove = cosine_sim(groove_org, groove)    
      ...
      #   reward_chroma = baseline['chroma']
      #   reward_groove = baseline['groove']
      ...
      row = {'inst_f1':reward_if1,'chord_f1':reward_cf1,'time_sig_acc':reward_ts,'pitch_oa':reward_poa,'velocity_oa':reward_voa,'duration_oa':reward_doa,'chroma_sim':reward_chroma,'groove_sim':reward_groove,'note_density_nsq_err':reward_nd}
      ...
      baseline['chroma'] = args.alpha * baseline['chroma'] + (1 - args.alpha) * reward_chroma
      baseline['groove'] = args.alpha * baseline['groove'] + (1 - args.alpha) * reward_groove
  ...
  return {'input_ids' : x,
          ...
          'description' : z_,
          'desc_bar_ids' : desc_bar_ids_}, reward_rule, reward_metric, is_done, bar_done, baseline, df
...
def cosine_sim(p_true, p_pred):
  min_length = min(len(p_true),len(p_pred))
  return np.sum(p_true[:min_length] * p_pred[:min_length])
...
def get_meta(bar_seq):
  ...
  if len(pitch_classes):
    chroma = np.bincount(pitch_classes, minlength=12)
    chroma = chroma / np.sqrt(np.sum(chroma ** 2))
  else:
    chroma = np.array([1/12] * 12)

  positions = [int(note[0].split('_')[-1]) for note in notes] 
  if len(positions):
    groove = np.bincount(positions, minlength=48)
    groove = np.convolve(groove, [1, 4, 1], 'same')
    groove = groove / np.sqrt(np.sum(groove ** 2))
  else:
    groove = np.array([1/48] * 48)  
  ...
  return note_density, pitch_mean, pitch_std, velocity_mean, velocity_std, duration_mean, duration_std, ts, inst, chord, chroma, groove 
...
def dataloader_setup(rank, world_size, model, coll, args):
  ...
  print(f"Test set size for rank_{rank}: {len(midi_files)}")
  ...
  try:
    ...
    for i in range(0, len(episode_memory), args.batch_size_opt):
      ...
      if args.baseline == "V":
        ...
        if num_bars < args.warm_up:
          continue
        base = baseline['nd'] + baseline['poa'] + baseline['voa'] + baseline['doa'] + baseline['ts'] + baseline['if1'] + baseline['cf1'] + baseline['chroma'] + baseline['groove']
...
def train(rank, world_size):
  ...
  memory = []
  baseline = {'nd':0.34, 'poa':0.71, 'voa': 0.61, 'doa': 0.47, 'ts': 0.99, 'if1': 0.95, 'cf1': 0.6, 'chroma':0.73, 'groove':0.66}
  ...
  for sequence_ in sequences:
    ...
    for bar_seq in bar_seqs:
      note_density, pitch_mean, pitch_std, velocity_mean, velocity_std, duration_mean, duration_std, time_signature, instruments, chords, chroma, groove = get_meta(bar_seq)
      ...
      meta_per_song['chroma'].append(chroma)
      meta_per_song['groove'].append(groove)
  ...
  for temp_idx in range(args.max_iter):
    ...
    next_state, reward_rule, reward_metric, terminated, bar_done, baseline, df = step(rank, state, action, z, desc_bar_ids, policy_net, vocab, terminated, events, bar_ids, current_ids, num_bars, meta_stats, baseline, df, args)
    ...
    for j,k in zip(range(batch_size),current_ids):
      ...
      if bar_done[j]:
        ...
        bar_length[j] = 1
        bar_no[j] += 1
        # if num_bars == args.test_size * args.max_bars:
        if num_bars % args.max_bars == 0:
          # df.to_csv('results.csv')
          # print(df)
          df_copy = df.copy(deep=True)
          df_copy['id'] = df_copy.index // args.max_bars
          summary = df_copy.groupby('id').mean().mean()
          nsq_err = df_copy.groupby('id')['note_density_nsq_err'].mean()
          summary['note_density_nrmse'] = np.sqrt(nsq_err).mean()
          summary = summary.drop(columns='note_density_nsq_err')
          print(f"Evaluation Results for {args.checkpoints_save}")
          print(summary)
    ...
    for _ in terminated:
      if _ == 1:
        ...
        events = pad_sequence(events, batch_first=True, padding_value=0).to(rank)
